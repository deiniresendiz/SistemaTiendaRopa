...
Public Class frmVentas
    ...
    Private Sub btnNuevo_Click(sender As Object, e As EventArgs) Handles btnNuevo.Click
        ...
        Dim r As String = "select count(*) from Ventas"
        comando = New SqlCommand(r, connection)
        textIdVenta.Text = comando.ExecuteScalar + 1
        textFactura.Text = Val(textIdVenta.Text).ToString("0000000")
    ...
    Sub guradarDatos()
        ...
        Try
            ...
            command.CommandText = "insert into Ventas(IdVenta,IdEmpleado,IdCliente,Folio,SubTotal,Iva,Total,FechaVenta,Estado) values (" & Val(textIdVenta.Text) & "," & Val(textIdEmpleadoE.Text) & "," & Val(textIdClienteC.Text) & ",'" & textFactura.Text & "','" & CDbl(textSubTotal.Text) & "','" & CDbl(textIva.Text) & "','" & CDbl(textTotal.Text) & "','" & fecha & "','R')"
            ...
            For x = 0 To dgDatosCompra.Rows.Count - 2
                ...
                command.CommandText = "insert into DetVentas(IdVenta,IdProducto,Cantidad,Precio) values(" & Val(textIdVenta.Text) & "," & d1 & "," & d3 & ",'" & c1 & "')"
                command.ExecuteNonQuery()
