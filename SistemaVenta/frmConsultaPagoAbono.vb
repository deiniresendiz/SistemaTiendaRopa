...
Public Class frmConsultaPagoAbono
    ...
    Private Sub frmConsultaPagoAbono_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        connection.Close()
        connection.Open()
    End Sub
    ...
    Sub consDetApartado(ByVal s As String)
        dgDatos.Rows.Clear()
        Dim r = "select * from Abonos where IdApartado =  '" + s + "';"
        comando = New SqlCommand(r, connection)
        lector = comando.ExecuteReader

        Dim x As Integer = 1
        While lector.Read
            dgDatos.Rows.Add(x, lector(1), lector(2), lector(3))
            x = x + 1
        End While
        lector.Close()
